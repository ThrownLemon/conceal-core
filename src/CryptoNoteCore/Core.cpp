...
namespace cn
{
  ...
  size_t core::addChain(const std::vector<const IBlock *> &chain)
  {
    ...
    for (const IBlock *block : chain)
    {
      ...
      {
        logger(ERROR, BRIGHT_RED) << "core::addChain() failed to handle incoming block " << get_block_hash(block->getBlock()) << ", " << blocksCounter << "/" << chain.size();
  ...
  bool core::handle_incoming_tx(const BinaryArray &tx_blob, tx_verification_context &tvc, bool keeped_by_block)
  { // Deprecated. Should be removed with CryptoNoteProtocolHandler.
    ...
    if (!ok)
      blockHeight = this->get_current_blockchain_height(); // this assumption fails for withdrawals
    ...
    for (size_t try_count = 0; try_count != 10; ++try_count)
    {
      ...
      if (coinbase_blob_size < cumulative_size - txs_size)
      {
        ...
        if (cumulative_size != txs_size + getObjectBinarySize(b.baseTransaction))
        {
          ...
          {
            logger(ERROR, BRIGHT_RED) << "unexpected case: cumulative_size=" << cumulative_size << " + 1 is not equal txs_cumulative_size=" << txs_size << " + get_object_blobsize(b.baseTransaction)=" << getObjectBinarySize(b.baseTransaction);
            return false;
          }
          ...
          if (cumulative_size != txs_size + getObjectBinarySize(b.baseTransaction))
          {
            ...
            logger(TRACE, BRIGHT_RED) << "Miner tx creation have no luck with delta_extra size = " << delta << " and " << delta - 1;

            cumulative_size += delta - 1;
            continue;
          }
          logger(DEBUGGING, BRIGHT_GREEN) << "Setting extra for block: " << b.baseTransaction.extra.size() << ", try_count=" << try_count;
        }
      }
  ...
  bool core::handle_incoming_block(const Block &b, block_verification_context &bvc, bool control_miner, bool relay_block)
  {
    ...
    if (relay_block && bvc.m_added_to_main_chain)
    {
      ...
      else
      {
        ...
        {
          logger(ERROR, BRIGHT_RED) << "can't find some transactions in found block:" << get_block_hash(b) << " txs.size()=" << txs.size() << ", b.transactionHashes.size()=" << b.transactionHashes.size() << ", missed_txs.size()" << missed_txs.size();
          return false;
        }
