...
void serialize(PqAccountPublicAddress& address, ISerializer& serializer) {
  ...
  // the input path, which would be a malleability/desync footgun. `kemPublicKey` length is validated
  // by the address parser (parsePqAccountAddressString), not here.
