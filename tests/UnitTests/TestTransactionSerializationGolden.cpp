...
{
  // A transaction that exercises every input/output variant together, so a desync in
  // getSignaturesCount or any variant serializer surfaces here.
  Transaction tx;
  tx.version = 3;
  tx.unlockTime = 42;
...
TEST(TransactionExtraGolden, EachFieldTypeRoundTrips)
{
  ...
  // Re-write the parsed fields; pubkey/nonce/mm/message/ttl all reproduce identical bytes.
