...
| **Deposit freeze (Option 3)** | classical (Ed25519) deposit creation frozen at/after V9 — PQ-only deposits after the fork; creation-side only (existing deposits stay spendable). 3 gates: `pushBlock` per-tx + coinbase, `add_tx` acceptance | `Blockchain.cpp` (`transactionContainsClassicalDeposit`), `TransactionPool.cpp`; see [`deposit-freeze-impl.md`](deposit-freeze-impl.md) |
